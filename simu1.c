...
int observed[INTERVALS] = {0}, expected, N=0;; // observed frequencies , expected frequency, total
int i,j, array[100]; //o store the random numbers generated
float final,calculation[20];
printf ("The random numbers are: \n") ;
for (i = 0; i < n; i++) // generate random numbers between 0 and 99 using m=100 and mixed congruently
{
x1= (a*x0+c) %m;
array[i]=x1;
x0=x1;
